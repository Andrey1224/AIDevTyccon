...
1. Run `npm run test` to verify that all Vitest unit tests pass.
2. Run `npm run build` to verify there are no TypeScript compilation or Vite bundling errors.
...
- **Game Engine**: A tick-based system (`tick()`) that advances game time and processes weekly changes (burn rate, project progress, etc.).
